...
## Qué me queda a medias
- Modos `x`, `r+`, `w+`, `a+` aún no practicados.
- `encoding="utf-8"` explícito en `open()` ya se ha usado y explicado; queda reforzarlo cuando aparezcan JSON o ficheros con tildes.
- `try/except FileNotFoundError` aplicado a `with open(...)` ya está practicado en `contar_basico`; queda repetirlo cuando se defina una nueva función con contrato de retorno.
- `try/except` aplicado a escritura de ficheros aún no practicado.
...
## Ajuste de método acordado (2026-06-18)
- Problema detectado: patrones como `sorted(key=lambda ...)` combinan varias piezas y necesitan repetición para automatizarse.
- Nueva estructura de clase por defecto: 5 min de mapa, 10 min de calentamiento, 25-35 min de avance nuevo, 5 min de cierre.
- Nueva regla de pistas: concepto -> estructura incompleta -> línea casi completa -> solución solo si Sergi la pide.
- Antes de avanzar a v0.3, hacer mini-ejercicio de top con diccionario pequeño para reforzar `items()`, tuplas, índices, `sorted`, `lambda`, `reverse` y slicing.

## Ajuste de infraestructura docente (2026-09-01)
- El repo queda adaptado a Codex sin borrar compatibilidad con Claude Code. ✅
- `AGENTS.md` pasa a ser el wrapper nativo de Codex; `CLAUDE.md` queda como wrapper de compatibilidad. ✅
- Añadido `memoria-aprendizaje/conceptos-python.md` para seguimiento ligero de conceptos y repasos. ✅
- Próximo micro-paso de clase no cambia: calentamiento `dict -> items() -> sorted(key=lambda ...) -> slicing`, y después `TextAnalyzer` v0.3 con `@medir_tiempo`. ✅
