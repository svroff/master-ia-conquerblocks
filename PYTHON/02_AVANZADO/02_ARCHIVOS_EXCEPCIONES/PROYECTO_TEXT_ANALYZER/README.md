...
Pequeño analizador de ficheros de texto en Python que cuenta líneas, palabras y caracteres, y muestra las palabras más frecuentes.
...
## ¿Qué tengo que hacer? (v0.1 y v0.2)

El objetivo de la primera versión era implementar una sola función: `contar_basico(ruta)`. La segunda versión añade `contar_palabras(ruta)` para obtener el top 5 de palabras más frecuentes.
...
## Y después de v0.2, ¿qué?

El proyecto sigue por capas, una por sesión o menos. **No intentes hacerlas todas a la vez.** Mejor cerrar una versión limpia que tener varias a medias.
...
| ✅ v0.2 | `contar_palabras(ruta)`: top 5 palabras más frecuentes | Practicar `dict` de conteo y `sorted(key=lambda...)` |
| ⏳ v0.3 | Decorador `@medir_tiempo` que envuelve la función principal | Llevar los decoradores a un caso real (no solo `@lru_cache`) |
| ⏳ v0.4 | `analizar_fichero(ruta)` con contrato de retorno claro | Combinar lo que ya sabes en una función de entrada robusta |
...
- **Cuando termines una versión, actualiza la tabla de arriba.**
...
Por ahora, el script mantiene una prueba directa al final del archivo. Más adelante, cuando toque módulos/imports, se reorganizará la entrada principal del programa.
